...
double ref_cte = 0;
double ref_epsi = 0;
double ref_v = 30 * 0.44704; //30mph in mps
double max_accel = 1.00;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, vector<double> &trk_x, vector<double> &trk_y) {
  bool ok = true;
  size_t i;
  ...
  //populate predictec trajectory
  for (int i = x_start; i < y_start; i++)
  {
    trk_x.push_back(solution.x[i]);
  }
  for (int i = y_start; i < psi_start; i++)
  {
    trk_y.push_back(solution.x[i]);
  }
